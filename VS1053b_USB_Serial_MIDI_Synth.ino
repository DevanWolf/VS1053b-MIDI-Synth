...
void midiByte(byte data1, byte data2, byte data3) {
	digitalWrite(XDCS, LOW);
	SPI_transfer8bit(data1);
	SPI_transfer8bit(data2);
	SPI_transfer8bit(data3);
...
void plugin(word plg[], word size) {
	word i = 0;
	while (i < size) {
			...
			if (data == 0xF7 and cmd == 0xF0) {
				if (sysex[1] == 0x7F) {
					if (sysex[0] == 0x7E and sysex[2] == 9 and sysex[3] >= 1 and sysex[3] <= 3 and value == 3) {
						...
						balance = 0;
						polymono = 0;
						if ((sysex[3] & 1) == GMmode) writeRegisterSplit(0x0A, 0, 0x50);
						else {
							GMmode = !GMmode;
							if (GMmode) plugin(GMon, GM_ON);
							else {
								reverbOff = false;
					...
					else if (sysex[3] == 3 and sysex[4] < 0x10 and value == 4 and GMmode) {
...
#ifdef REVERB_TOGGLE
	if (!digitalRead(REVERB_TOGGLE) and GMmode and !debounce) {
		debounce = true;
#ifdef LED
		digitalWrite(LED, HIGH);
#endif
