...
// If you're using a different Arduino and/or a different music shield, change the below 3 pin IDs to their matching connections
#define XCS A3
#define XDCS A2
#define XRESET A0
...
void writeRegister(byte address, word value) {
	writeRegisterSplit(address, highByte(value), lowByte(value));
}
...
#ifdef REVERB_TOGGLE
void reverb(byte setting) {
	writeRegisterSplit(7, 0x1e, 3);
	writeRegisterSplit(6, 0, setting);
}
#endif
...
void setup() {
	...
	delayMicroseconds(1792);

	for (word i = 0; i <= plgSize; i++) writeRegister(i == 0 ? 7 : i == plgSize - 1 ? 0x0A : 6, pgm_read_word(&plg[i]));
...
void loop() {
	while (Serial.available()) {
		...
		if (data >> 5 == 4 or data >> 4 == 11 or data >> 4 == 12 or data >> 4 == 14 or data == 0xF0) {
			...
			if (cmd >> 5 == 4 or cmd >> 4 == 11 or cmd >> 4 == 12 or cmd >> 4 == 14) {
				if (value < 0 and cmd >> 5 != 6) value = data;
				else {
					if (!(cmd >> 4 == 11 and ((value >= 1 and value <= 5) or value == 8 or value == 9 or (value >= 13 and value <= 37) or value == 65 or (value >= 67 and value <= 90) or (value >= 92 and value <= 97) or (value >= 102 and value <= 119) or value == 122 or value >= 126))) { // filter out unassigned CCs
						digitalWrite(XDCS, LOW);
						SPI_transfer8bit(cmd);
						if (cmd >> 5 != 6) SPI_transfer8bit(value);
			...
			if (data == 0xF7 and cmd == 0xF0) {
				if (sysex[1] == 0x7F) {
					if (sysex[0] == 0x7E and sysex[2] == 9 and (sysex[3] == 1 or sysex[3] == 3) and value == 3) {
						volume = 0;
						resetMIDI();
					} else if (sysex[0] == 0x7F and sysex[2] == 4) {
						if (sysex[3] == 1) {
							volume = 127 - sysex[5];
							updateVolume();
						} else if (sysex[3] == 2) {
				...
				} else if (!sysex[0] and sysex[1] == 1 and sysex[2] == 0x11) {
					if (sysex[3] == 1 and sysex[4] < 4 and value == 4) writeRegisterSplit(0, 8, (sysex[4] & 1) << 4 | (sysex[4] & 2) << 6);
					if (sysex[3] == 2) writeRegisterSplit(2, sysex[5], sysex[4]);
...
#endif
#if defined(VOLUME_UP) && defined(VOLUME_DOWN)
#if defined(REVERB_TOGGLE) || defined(RESET)
	else
#endif
	if (!digitalRead(VOLUME_UP) and volume) {
		--volume;
		volumeControl();
	} 
